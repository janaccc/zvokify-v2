...
export default function MusicPlayer() {
    ...
    // Posodobi currentTime in duration
    useEffect(() => {
        const audio = audioRef.current;
        if (!audio) return;
        ...
        <div className="fixed bottom-0 left-0 w-full bg-black text-white px-4 py-3 shadow-md z-50">
            ...
            <div className="max-w-8xl w-[95%] mx-auto flex items-center justify-between">
                    ...
                    {currentSong && (
                        <>
                            ...
                            <div className="text-sm">
                                ...
                                <p className="text-secondary-text font-normal">{currentSong.artist}</p>
                            </div>
                        </>
                    )}
                </div>

                {/* Kontrole predvajanja in skip 5 sekund */}
                <div className="max-w-[400px] w-full flex items-center flex-col gap-3">
                    <div className="flex gap-4">
                        {/* Skip backward 5 sekund */}
                        <button
                            onClick={() => {
                                if (!audioRef.current) return;
                    ...
                    <div className="w-full flex justify-center items-center gap-2">
                        ...
                        <div className="w-full">
                            <input
                                ...
                                max={duration}
                                value={currentTime}
                                className="w-full outline-none h-1 bg-zinc-700 rounded-md appearence-none accent-white"
                            />
                        </div>
                ...
                <div className="flex items-center gap-2">
                    {volume === 0 ? (
                        <button onClick={toggleMute} className="text-secondary-text text-xl cursor-pointer hover:bg-zinc-600 rounded-md">
                            <IoMdVolumeOff />
                        </button>
                    ) : (
                        <button onClick={toggleMute} className="text-secondary-text text-xl cursor-pointer hover:bg-zinc-600 rounded-md">
